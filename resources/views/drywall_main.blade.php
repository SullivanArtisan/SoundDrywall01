...
@section('function_page')
    <div>
        <div class="row m-4">
            <div>
				<h2 class="text-muted pl-2">Drywall Sheets</h2>
            </div>
            <div class="col my-auto ml-5">
				<button class="btn btn-success mr-4" type="button"><a href="{{route('drywall_add')}}">Add</a></button>
			</div>
            <div class="col">
                <!--
	...
	<?php
		...
		if ($needResort == true) {
			...
			$materials = \App\Models\Material::where('mtrl_type', 'DRYWALL SHEET')->where('mtrl_status', '<>', 'DELETED')->orderBy($_GET['sort_key_material'], session('sort_order', 'asc'))->paginate(10);
			session(['sort_key_material' => 'mtrl_name']);
		} else {
			$materials = \App\Models\Material::where('mtrl_type', 'DRYWALL SHEET')->where('mtrl_status', '<>', 'DELETED')->orderBy($sortKey, $sortOrder)->paginate(10);
		}
        ...
        $outContents .= "<div class=\"row bg-info text-white fw-bold\">";
			$outContents .= "<div class=\"col-3 align-middle\">";
				...
				$outContents .= "<a href=\"drywall_main".$sortParms."\">";
				$outContents .= "Name";
				if ($sort_icon == 'asc') {
		...
		foreach ($materials as $material) {
            $outContents = "<div class=\"row\">";
				$outContents .= "<div class=\"col-3\">";
					$outContents .= "<a href=\"drywall_selected?id=$material->id\">";
